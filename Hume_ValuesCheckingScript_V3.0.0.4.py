...
FEATURE_CLASS   = 'C:\\data\\humecheck\\WandySpecial_DWP_Nov.shp' # Input Feature Class
FEATURE_ID      = 'dap_ref_no'                     # Feature ID Field
THEME_REFTAB    = 'C:\\data\\daptest\\Single Report Tool\\Reference Tables\\reftables.gdb\\REFTABLE_DAP_20250716'  # Theme Reference Table
GISPUB_PATH     = 'C:\\data'                       # gis_public folder location
OUT_PATH        = 'C:\\data\\humecheck\\Outputs2'   # Output Path
...
class ValuesCheckTool:
    ...
    def _results_to_string(self, list_poly: list, list_buff: list, method: str, buffer_distance: int, geometry_type: str) -> List:
        ...
        try:
            ...
            for location in locations:
                ...
                if not any(input):
                    output.append("Nil features") # populate with string "Nil features" if list empty
                else:
                    for value in input:   
                        if method.upper() == "PRESENT":
                            # set default value string
                            val_str = "Nil features"
            ...
            if buffer_distance > 0:
                values_string = (
                    '\r\n'.join(str(item) for item in attrs_poly) +
                    f'\r\nWithin Buffer Area ({buffer_distance}m):' +
    ...
    def load_values_fcs(self, reftab: str) -> None:
        ...
        try:
            ...
            theme_fields = ["OID@", "THEMENAME", "CHECK_YN", "DEFAULTWS_YN", "DATA_LOC", "GDB_NAME", "FC_NAME",
            ...
            with arcpy.da.SearchCursor(reftab, theme_fields) as cursor:
                for row in cursor:
                    ...
                    (row_oid, theme_name, requires_check, default_ws, location, gdb_name, fc_name, 
                        query, method, repfld1, repfld2, repfld3, repfld4, 
                        buffer_distance) = row

                    # Skip to next iteration if check not required
                    ...
                    # Check if the feature class exists before trying to use it
                    if not arcpy.Exists(values_fc_path):
                        self.logMessage('warning', f"Feature class not found: {values_fc_path}")
                        continue
                    
                    # Cache values layer with unique name
                    if fc_name not in self.values_cache:
                        layer_name = f"{fc_name}_{row_oid}"
